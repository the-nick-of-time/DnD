...
class toplevel:
    def __init__(self,window):
        ...
        self.roll.grid(2,0)
        self.exit=tk.Button(window,text="QUIT",fg="red",command=lambda:self.writequit())
    ...
    def getConfigData(self):
        ...
        try:
            ...
            self.characterdata['spell slots']=r.readList(data['spell slots'],mode='int')
            return data
        
        except(KeyError):
            errorWindow=tk.Tk()
            tk.Label(errorWindow,text="There is no .ini file in the current directory\nwith name "+ini_name).pack()
            tk.Button(errorWindow,command=errorWindow.destroy,text="OK").pack()
            return None
    ...
    def initCharacter(self):
        abils=self.pullABIL()
        info=self.pullINFO()
        self.activeCharacter=dnd.character(info['name'],int(info['level']),abils,self.characterdata['Attacks'],info['class'],int(info['caster level']),self.characterdata['spell slots'])
        self.spell.draw()
    ...
    def writequit(self):
        ...
        try:
            ...
            writer['Spell Slots']['spell slots']=str(self.activeCharacter.spellslots)
            writer['Character']['level']=str(self.activeCharacter.level)
            writer['Character']['caster level']=str(self.activeCharacter.casterLevel)
            for (i,n) in enumerate(['str','dex','con','int','wis','cha']):
...
class infosec:
    def __init__(self,master,app):
        ...
        self.load=tk.Button(buttons,text="Load character\nfrom file",command=lambda:self.LOAD())
        self.load.grid(row=0,column=0)
        self.initialize=tk.Button(buttons,text="Initialize\ncharacter",command=lambda:self.INITIALIZE())
...
class spellsec:
    def __init__(self,master,app):
        ...
        self.reset=tk.Button(self.f,text="Reset\nSpells",command=lambda: self.RESET())

    def draw(self):
        char=self.top.activeCharacter
        i=1
        while(i<len(char.spellslots)):
            self.spellDisplays[i]["text"]="Spells left of level "+str(i)+": "+str(char.spellslots[i])+"     "
...
class abilsec:
    def __init__(self,master,app):
        self.parent=master
        self.f=tk.Frame(master)
        self.calc=tk.Button(self.f,text="Calculate\nModifiers",command=lambda:self.calcModifiers())
        ...
        for (i,l) in enumerate(abilitynames):
            self.entries.append(util.labeledEntry(self.f,l,i,0,orient='h',width=4))
...
class hpsec:
    def __init__(self,master,app):
        ...
        self.amount=util.labeledEntry(self.f,'HP Change\nas integer or roll',2,0,width=10,orient='h')
        self.temp=util.labeledEntry(self.f,'+Temporary HP',0,2,width=5)
        buttons=tk.Frame(self.f,pady=15)
        buttons.grid(row=2,column=2)
        self.alterHP=tk.Button(buttons,text='Alter\nHP',command=lambda:self.changeHP(False))
        self.alterHP.grid(row=0,column=0)
        self.addtemp=tk.Button(buttons,text='Add to\ntemp HP',command=lambda:self.changeHP(True))
...
class attacksec:
    def __init__(self,master,app):
        ...
        hasAdvantage=tk.Checkbutton(self.f,text="Advantage?",variable=self.adv)
        hasAdvantage.grid(row=0,column=1,sticky="w")
        hasDisadvantage=tk.Checkbutton(self.f,text="Disadvantage?",variable=self.dis)
        hasDisadvantage.grid(row=1,column=1,sticky="w")

        bonuses=tk.Frame(self.f,pady=5)
        bonuses.grid(row=2,column=0)
        self.attackBonus=util.labeledEntry(bonuses,"Attack bonus",0,0,orient='h',width='8')
        self.damageBonus=util.labeledEntry(bonuses,"Damage bonus",1,0,orient='h',width='8')

        self.do=tk.Button(self.f,text="ATTACK",command=lambda: self.doAttack())
    ...
    def doAttack(self):
        try:
            res=dnd.resultFormat(self.top.activeCharacter.attacks[self.attack.get().casefold()].attack(self.top.activeCharacter, self.adv.get(), self.dis.get(), self.attackBonus.get(), self.damageBonus.get()))
...
class rollsec:
    def __init__(self,master,app):
        ...
        self.button=tk.Button(self.f,text="ROLL",command=lambda: self.doRoll())
