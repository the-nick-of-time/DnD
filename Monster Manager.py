...
class charbuild:
    def __init__(self,master):
        ...
        self.finish=tk.Button(self.win,text="Finish",command=lambda: self.end())
...
class charsub:
    def __init__(self,master,character):
        ...
        self.name.pack()
        self.init=tk.Label(self.f,text=character.initiative,font="Calibri 14")
        self.init.pack()
...
class sub:
    def __init__(self,master,monster):
        ...
        self.hpL=tk.Label(hpLsec,text='HP:  '+str(monster.HP),font='Calibri 18')
        self.hpL.grid(row=0,column=0,sticky='w')
        self.acL=tk.Label(hpLsec,text='   AC:  '+monster.AC,font='Calibri 16')
        ...
        hpdo=tk.Button(hpmain,text='Change',command=lambda:self.changeHP())
        ...
        self.attackE=labeledEntry(attacknotresult,'Attack Modifiers',0,0,orient='h',pos='w',width=15)

        self.damageE=labeledEntry(attacknotresult,'Damage',1,0,orient='h',pos='w',width=15)

        perform=tk.Button(attacknotresult,text="Attack",command=lambda: self.doattack())
        ...
        hasAdvantage=tk.Checkbutton(advsec,text="Advantage?",
                                         variable=self.advantage)
        hasAdvantage.grid(row=0,column=1,sticky="w")
        hasDisadvantage=tk.Checkbutton(advsec,text="Disadvantage?",
        ...
        for i in range(len(abilnames)):
            tk.Label(abilframe,text=abilnames[i]).grid(row=(i//3)*2,column=i%3)
            tk.Label(abilframe,text=str(self.creature.abilities[i])+" ("+str(modifier(self.creature.abilities[i]))+")").grid(row=(i//3)*2+1,column=i%3)
...
class builder:
    ...
    def __init__(self,master):
        ...
        self.average=tk.Checkbutton(self.win,text="Take average?",variable=self.av)
...
class main:
    ...
    def __init__(self,window):
        ...
        self.makenew=tk.Button(self.buttons,text='New Monster',command=lambda: self.create())
        self.makenew.grid(row=0,column=0)
        self.makechar=tk.Button(self.buttons,text='New Character',command=lambda: charbuild(self))
        self.makechar.grid(row=0,column=1)
        self.qa=tk.Button(self.buttons,text='Quit',command=lambda: self.end())
...
##        self.qa.grid(row=5,column=abs(len(self.frames)-1)//6)
    ...
    def pull(self,otherwin):
        ...
        hp=r.call(otherwin.hp.get(),option='average') if otherwin.av.get() else r.call(otherwin.hp.get())
        rv=monster(otherwin.name.get(),hp,otherwin.hp.get(),r.call('1d20',modifiers=modifier(abils[1])),otherwin.ac.get(),abils)
