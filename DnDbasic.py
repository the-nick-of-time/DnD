...
class character:
    ...
    def __init__(self, name, level, abilities, attacks={}, Class='multiclass',
                 casterLevel=0, spellSlots=[]):
        #name: string
        ...
        if(Class == 'bard' or Class == 'cleric' or Class == 'druid' or Class == 'sorcerer' or Class == 'wizard' or Class == 'multiclass'):
...
class spell:
    def __init__(self, level, ability, damageDice, addAbilToDamage=False,
                 attackRoll=False, savingthrowtype='', multipleAttack=1,
                 effects=''):
    ...
    def attack(self, character, adv, dis, attackBonus, damageBonus):
        ...
        for all in range(self.multiple):
            ...
            if(self.attackroll):
                if(adv and not dis):
                    ...
                    attackstring='1d20'
                AB=r.call(attackBonus)+character.proficiencyBonus+modifier
                ...
                if(attackresult==1):
                    ...
                    opt='execute'
            DB=r.call(damageBonus,option=opt)+(modifier if self.addabiltodamage else 0)
            if(self.level == 0):
                dice=self.damagedice+('+'+self.damagedice)*(character.cantripscale-1)
            else:
        ...
        if(not self.attackroll):
            ...
            result[0]=('Make a DC '+str(saveDC)+' '+self.savetype+' saving throw.')
        result[2]=self.effects
        return result

class weapon:
    def __init__(self, damageDice, ability, multipleAttack=1, magicBonus=0,
                 attackRoll=True, ammunition=-1, effects=''):
        ...
        for all in range(self.multiple):
            ...
            if(self.attackroll):
                if(adv and not dis):
                    ...
                    attackstring='1d20'
                AB=r.call(attackBonus)+character.proficiencyBonus+modifier+self.magicbonus
                attackresult=r.call(attackstring)
                if(attackresult==1):
                    result[0].append('Miss')
                    opt='zero'
